 ...
 * PHP version 5.4
 */
class DraftmediaVisitsEnable extends \Core\Model
{
    static $DB_TABLE_NAME = 'draftmedia_visits_enable';
    static $TABLE_VALUES =
        [
            0 => "id",
            1 => "enable"
    ...
    public static function persistAndFlush($object)
    {
        ...
        if ($data[0] == null) {
            //insert
            try {
                $resultadoPreparado = $db->prepare("INSERT INTO $thisTable($tv[0],$tv[1]) VALUES (:$tv[0],:$tv[1])");
            ...
            //update
            try {
                $existingObject = DraftmediaVisitsEnable::find($data[0])[0];
                $i=0;
                $existingData=[];
