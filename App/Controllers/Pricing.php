 ...
 * PHP version 5.4
 */
class Pricing extends \Core\Controller
{
    ...
    protected function before()
    {
        ...
        if ($ddos->isThereAnAttack(10)) {
            ...
            try {
                $user = $this->getUser();
                $hA->isHackAttempt("pricing", "before", $user);
            } catch (\Exception $e) {
                $hA->isHackAttempt("pricing", "before", "");
            }
     ...
     */
    public function ajaxPricingIndexVisitsAction() {
        ...
        if ($visitors->isDraftmediaVisitsEnabled()) {
            ...
            if (isset($_SESSION["user"]) || isset($_SESSION["key"])) {
                ...
                if ($user['role'] != 'admin') {
                    ...
                    $draftmediaVisits[$n]['Pricing_index_auth_visits']++;
                ...
                $draftmediaVisits = $DMV::findAll();
                $n = count($draftmediaVisits)-1;
                $draftmediaVisits[$n]['Pricing_index_anonym_visits']++;
                $DMV::persistAndFlush($draftmediaVisits[$n]);
            }
        }
    }
     ...
     * DATE November 17, 2018
     */
    public function ajaxPricingIndexTimeAction() {
        $visitors = new Visitors();
        if ($visitors->isDraftmediaVisitsEnabled()) {
            $visitors->checkDraftmediaDate();
            if (isset($_SESSION["user"]) || isset($_SESSION["key"])) {
                ...
                if ($user['role'] != 'admin') {
                    ...
                    $n = count($draftmediaVisits)-1;
                    for ($i=0; $i<5; $i++) {
                        $draftmediaVisits[$n]['Pricing_index_auth_time']++;
                    }
                    $DMV::persistAndFlush($draftmediaVisits[$n]);
                }
            } else {
                ...
                $n = count($draftmediaVisits)-1;
                for ($i=0; $i<5; $i++) {
                    $draftmediaVisits[$n]['Pricing_index_anonym_time']++;
                }
                $DMV::persistAndFlush($draftmediaVisits[$n]);
            }
        }
    ...
    public function indexAction()
    {
        ...
        if (isset($_COOKIE['1D5M9_7L5a3n0'])) {
            ...
            if ($language=='english' || $language=='spanish') {
                ...
                $hA = new CoreHackAttempt();
                $hA->isHackAttempt("pricing", "index", $this->getUser());
        ...
        View::renderTemplate('Pricing/index.html.twig', [
