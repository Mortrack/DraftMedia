...
class Aboutus extends \Core\Controller
{
    ...
    protected function before()
    {
        ...
        if ($ddos->isThereAnAttack(10)) {
            ...
            try {
                $user = $this->getUser();
                $hA->isHackAttempt("aboutus", "before", $user);
            } catch (\Exception $e) {
                $hA->isHackAttempt("aboutus", "before", "");
            }
     ...
     */
    public function ajaxAboutusIndexVisitsAction() {
        ...
        if ($visitors->isDraftmediaVisitsEnabled()) {
            ...
            if (isset($_SESSION["user"]) || isset($_SESSION["key"])) {
                ...
                if ($user['role'] != 'admin') {
                    ...
                    $draftmediaVisits[$n]['Aboutus_index_auth_visits']++;
                ...
                $draftmediaVisits = $DMV::findAll();
                $n = count($draftmediaVisits)-1;
                $draftmediaVisits[$n]['Aboutus_index_anonym_visits']++;
                $DMV::persistAndFlush($draftmediaVisits[$n]);
            }
        }
    }
     ...
     * DATE November 17, 2018
     */
    public function ajaxAboutusIndexTimeAction() {
        $visitors = new Visitors();
        if ($visitors->isDraftmediaVisitsEnabled()) {
            $visitors->checkDraftmediaDate();
            if (isset($_SESSION["user"]) || isset($_SESSION["key"])) {
                ...
                if ($user['role'] != 'admin') {
                    ...
                    $n = count($draftmediaVisits)-1;
                    for ($i=0; $i<5; $i++) {
                        $draftmediaVisits[$n]['Aboutus_index_auth_time']++;
                    }
                    $DMV::persistAndFlush($draftmediaVisits[$n]);
                }
            } else {
                ...
                $n = count($draftmediaVisits)-1;
                for ($i=0; $i<5; $i++) {
                    $draftmediaVisits[$n]['Aboutus_index_anonym_time']++;
                }
                $DMV::persistAndFlush($draftmediaVisits[$n]);
            }
        }
    ...
    public function indexAction()
    {
        ...
        if (isset($_COOKIE['1D5M9_7L5a3n0'])) {
            ...
            if ($language=='english' || $language=='spanish') {
                ...
                $hA = new CoreHackAttempt();
                $hA->isHackAttempt("aboutus", "index", $this->getUser());
        ...
        View::renderTemplate('AboutUs/index.html.twig', [
